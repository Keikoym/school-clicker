...
func _run() -> void:
	...
	verificar(main.qi == 4860 and main.obter_ganho_clique() == 2, "Livro comprado pelo botão aplica custo e bônus")
	colecao.comprar_item("casmurro")
	verificar(main.qi == 4860 and colecao.bonus_clique == 1, "Compra duplicada bloqueada")
	colecao.comprar_item("inexistente")
	verificar(main.qi == 4860, "ID desconhecido não altera saldo")
	for id in ["macunaima", "cortico", "caderno", "estojo", "luminaria"]:
		colecao.comprar_item(id)
	verificar(colecao.adquiridos.size() == 6 and main.qi == 2760, "Seis compras cobram os custos definidos")
	...
	verificar(main.qi == 2769, "Itens produzem sem apontador")
