...
const CATALOGO := [
	{"id": "casmurro", "nome": "Dom Casmurro", "tipo": "Livros", "custo": 120, "clique": 1, "passivo": 0, "nota": "Machado de Assis • Realismo\nInvestiga ciúme, memória e relações sociais."},
	{"id": "macunaima", "nome": "Macunaíma", "tipo": "Livros", "custo": 350, "clique": 2, "passivo": 0, "nota": "Mário de Andrade • Modernismo\nMistura lendas, fala popular e cultura brasileira."},
	{"id": "cortico", "nome": "O Cortiço", "tipo": "Livros", "custo": 700, "clique": 0, "passivo": 4, "nota": "Aluísio Azevedo • Naturalismo\nExplora a influência do meio sobre as pessoas."},
	{"id": "caderno", "nome": "Caderno de resumos", "tipo": "Materiais", "custo": 90, "clique": 1, "passivo": 0, "nota": "Organize as ideias e fortaleça seus cliques."},
	{"id": "estojo", "nome": "Estojo completo", "tipo": "Materiais", "custo": 280, "clique": 0, "passivo": 2, "nota": "Tudo à mão para estudar: produção automática."},
	{"id": "luminaria", "nome": "Luminária de estudo", "tipo": "Materiais", "custo": 900, "clique": 3, "passivo": 3, "nota": "Uma mesa preparada para longas leituras."}
]
const MEDALHAS := [
...
func _montar_interface() -> void:
	...
	coluna.add_child(_texto("Compra única • Bônus somados após multiplicadores • Esc: voltar", 11))
	painel.hide()
	aviso = _texto("", 14)
...
func _selecionar_aba(nome: String) -> void:
	...
	if aba == "Conquistas":
		for medalha in MEDALHAS:
			var progresso := mini(_progresso(medalha[0]), int(medalha[3]))
			var prefixo := "CONQUISTADA" if conquistas.has(medalha[0]) else "%d/%d" % [progresso, medalha[3]]
			lista.add_child(_texto("%s  •  %s\n%s" % [prefixo, medalha[1], medalha[2]]))
	elif aba == "Revisão":
		...
		for item in CATALOGO:
			...
			info.add_child(nota)
			info.add_child(_texto("+%d QI/clique   +%d QI/s" % [item.clique, item.passivo], 12))
...
func atualizar() -> void:
	...
	for item in CATALOGO:
		if botoes.has(item.id):
			...
			botao.text = "Adquirido ✓" if comprado else "%d QI" % item.custo
			botao.disabled = comprado or main.qi < int(item.custo)
			botao.tooltip_text = "Já está na sua coleção" if comprado else "Faltam %d QI" % maxi(0, int(item.custo) - main.qi)
